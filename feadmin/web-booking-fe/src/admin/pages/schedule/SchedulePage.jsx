    ...
    axios
      ...
      .then((response) => {
        if (Array.isArray(response.data)) {
          ...
          console.error('Data không phải là mảng:', response.data);
          toast.error("Dữ liệu không hợp lệ");
        }
      })
      .catch((error) => {
        console.error(error);
        toast.error("Lấy dữ liệu lịch chiếu thất bại");
      });
  }, []);
        ...
        <div className="overflow-x-auto">
          <table className="w-full table-auto border-collapse border border-gray-300 bg-white shadow-md">
            <thead className="bg-gray-200 text-gray-700 uppercase text-sm">
              <tr>
                <th className="border border-gray-300 px-4 py-2 text-left">Tên phòng</th>
                <th className="border border-gray-300 px-4 py-2 text-center">Tên phim</th>
                <th className="border border-gray-300 px-4 py-2 text-center">Ngày</th>
                <th className="border border-gray-300 px-4 py-2 text-center">Bắt đầu - kết thúc</th>
            ...
            <tbody>
              {Array.isArray(showList) && showList.length > 0 ? (
                ...
                <tr>
                  <td colSpan="5" className="text-center py-4">Không có lịch chiếu</td>
                </tr>
              )}
            </tbody>
          </table>
...
const ScheduleDisplayItem = ({
  ...
  return (
    <tr className="hover:bg-gray-100">
      ...
      <td className="border border-gray-300 px-4 py-2 text-center">
        <TimeDisplay isoDate={time_start} /> - <TimeDisplay isoDate={time_end} />
      </td>
      <td className="border border-gray-300 px-4 py-2 text-center">
